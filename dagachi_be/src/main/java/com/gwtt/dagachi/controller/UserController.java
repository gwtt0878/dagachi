...
public class UserController {
  ...
  public ResponseEntity<PagedModel<PostingSimpleResponseDto>> getAuthoredPostingsByUserId(
      @PathVariable Long id,
      @PageableDefault(size = 5, sort = "createdAt", direction = Sort.Direction.DESC)
          Pageable pageable) {
    return ResponseEntity.ok(
        new PagedModel<>(userService.getAuthoredPostingsByUserId(id, pageable)));
  }

  @GetMapping("/{id}/joined")
  public ResponseEntity<PagedModel<PostingSimpleResponseDto>> getJoinedPostingsByUserId(
      @PathVariable Long id,
      @PageableDefault(size = 5, sort = "createdAt", direction = Sort.Direction.DESC)
          Pageable pageable) {
    return ResponseEntity.ok(new PagedModel<>(userService.getJoinedPostingsByUserId(id, pageable)));
  }
}
